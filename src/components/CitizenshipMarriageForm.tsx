// src/components/CitizenshipMarriageForm.tsx
"use client";
...
function t(locale: Locale) {
  ...
  return {
    title: it
      ? "Prenota il supporto cittadinanza per matrimonio"
      : "Book citizenship-by-marriage support",
    desc: it
    ...
    optionGuideBody: it
      ? "Guida approfondita (PDF/online) con regole, tempistiche e documenti tipici per il percorso matrimonio, più checklist e modelli di email."
      : "Detailed written guide (PDF/online) with rules, timelines and typical documents for the marriage route, plus checklists and email templates.",
    optionCheckTitle: it
      ? "£170 – controllo documenti + piano 1:1"
      : "£170 – 1:1 document & plan check",
    optionCheckBody: it
      ? "Revisione pratica della tua situazione e dei documenti, call 1:1, piano scritto e struttura cartelle digitale."
      : "Practical review of your situation and documents, 1:1 call, written plan and simple digital folder structure.",
...
export default function CitizenshipMarriageForm() {
    ...
    try {
      ...
      await fetch(GAS_URL, {
        ...
        body: JSON.stringify({
          ...
          action: "submit",
          bookingId: id,
          service: `citizenship-by-marriage-${option}`,
          name: fullName.trim(),
          email: email.trim(),
      ...
      // Ask our API for a PaymentIntent
      const res = await fetch("/api/citizenship-by-marriage/create-intent", {
        method: "POST",
    ...
    try {
      ...
      const returnUrl =
        typeof window !== "undefined"
          ? `${window.location.origin}/services/citizenship-by-marriage?paid=1`
          : "https://www.resinaro.com/services/citizenship-by-marriage?paid=1";
