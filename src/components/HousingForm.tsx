...
export default function HousingForm() {
  ...
  const [issueType, setIssueType] = useState("Listing verification");
    ...
    try {
      ...
      const r1 = await fetch(GAS_URL, {
        ...
        body: JSON.stringify({
          ...
          files: filesPayload,
          data: { issueType, notes: notes.trim() },
        }),
      });
    ...
    <form onSubmit={handleSubmit} className="space-y-4" aria-live="polite">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <select value={issueType} onChange={(e) => setIssueType(e.target.value)} className="mt-1 block w-full rounded border px-3 py-2">
            <option>Listing verification</option>
            <option>Tenancy rights</option>
            <option>Deposit protection</option>
            <option>Council housing application</option>
            <option>Other</option>
          </select>
        </div>
        <div>
          <label className="block text-sm font-medium">Notes</label>
          <input value={notes} onChange={(e) => setNotes(e.target.value)} className="mt-1 block w-full rounded border px-3 py-2" placeholder="Briefly describe your issue" />
        </div>
      </div>
      <div>
      ...
      <div className="pt-2">
        <button type="submit" disabled={submitDisabled} className={`px-4 py-2 rounded text-white ${submitDisabled ? "bg-gray-400 cursor-not-allowed" : "bg-green-900 hover:bg-green-800"}`}>
          {submitting ? "Submitting..." : "Submit housing request + Pay"}
        </button>
