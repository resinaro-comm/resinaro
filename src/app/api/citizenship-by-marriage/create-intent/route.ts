// src/app/api/citizenship-by-marriage/create-intent/route.ts
...
const DESCRIPTIONS: Record<Option, string> = {
  "guide-35":
    "Italian citizenship by marriage guide (UK) — Resinaro",
  "check-170":
    "Italian citizenship by marriage 1:1 document & plan check (UK) — Resinaro",
};
...
export async function POST(req: NextRequest) {
  try {
    ...
    const paymentIntent = await stripe.paymentIntents.create({
      ...
      metadata: {
        service: "citizenship-by-marriage",
        option,
        name: name || "",
        email: email || "",
        telephone: telephone || "",
      ...
      automatic_payment_methods: {
        enabled: true,
        allow_redirects: "always", // for Klarna / Clearpay etc.
      },
    });
    ...
    console.error("Error creating citizenship-by-marriage PaymentIntent", err);
    return NextResponse.json(
